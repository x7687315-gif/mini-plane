...
| 2026-09-17 | **冻结**（前端 Sprint 4 实现并确认）。补充两条前端落地后才能确认的约定：① `created` / `deleted` 记录的 `new_value` / `old_value` 快照**前端不展开**，时间线只出动词短语（快照仍随载荷下发，供需要时取用）；② 映射表未覆盖的 `entity_type`（`state`/`label`/`workspace`/`member`）前端降级成 `{actor} 创建了{entity_type}`，**不渲染空行** | 已冻结 |
