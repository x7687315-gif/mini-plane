...
> 状态：**已冻结**（前端 Sprint 4 已按本契约实现并确认，2026-09-17）
...
|------|------------|
| 评论作者（**不论角色，含 Viewer**） | ✅ |
| 非作者，且生效角色 = Admin（项目 Admin，或 WS Admin 视同） | ✅（可以管理他人评论） |
| 非作者，且生效角色 = Member | ❌ **403** |
| 非作者，且生效角色 = Viewer | ❌ 403 |
| 该评论属于别的 Issue / 不存在 | **404** |
| 非工作区成员 | **404** |

> **「作者」优先于「角色」**（2026-09-17 澄清）：后端实现为
> `if comment.author_id != request.user.id: _require_role(role, ADMIN)`
> —— 是作者就**完全不进行角色判定**，因此 Viewer 可以修改/删除**自己写的**评论；
> 角色门槛只约束"管理他人评论"。前端 `types/comment.ts::canManageComment` 与之一致。
> 旧版表格把「作者本人」与「Viewer ❌」并列，容易被读成"Viewer 连自己的评论都不能删"，故拆开写明。
...
| 2026-09-17 | **冻结**（前端 Sprint 4 实现并确认）。澄清「作者优先于角色」：Viewer 可改删**自己**的评论，角色门槛只约束管理他人评论；权限表按「非作者」重新分行 | 已冻结 |
