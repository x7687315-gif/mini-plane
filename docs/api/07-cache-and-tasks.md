...
> 状态：**已冻结**（前端 Sprint 6 已按本契约实现并确认「二、异步任务」部分，2026-09-17）
> 公共约定见 [00-conventions.md](00-conventions.md)。
> 计划依据：[BACKEND_PLAN.md](../../BACKEND_PLAN.md) §Sprint 6。
> 本文回答三个问题：**什么被缓存了？什么时候失效？异步任务怎么查进度？**
>
> **前端确认范围（2026-09-17）**：本契约分两半 ——「一、缓存」是纯服务端行为，
> 前端只是普通消费者，没有可确认的实现面（仅实测了 `PATCH 改名 → 立即 GET 拿到新值`
> 无陈旧窗口这一条对用户可见的承诺）；「二、异步任务」已完整实现并验收
> （202 → TaskRun → 轮询 1s / 上限 30 次 → 完成后作废 issue 与活动缓存）。
...
| 2026-09-17 | **冻结**（前端 Sprint 6 实现并确认「异步任务」部分）：`POST …/issues/bulk/labels/` → 202 + TaskRun → 轮询 `GET …/tasks/{id}/`（1s / 上限 30 次）；`result.changed` 与 `result.issues` 在 UI 上是**两句话**（"变了 3 个"≠"3/5 成功"），前端已区分；批量后作废 issue 与两份活动缓存（§2.1「与单条 PATCH 同语义」的前端侧对应动作）。「一、缓存」为纯服务端行为，前端无实现面 | 已冻结 |
